...
class Acl extends Component
{
    ...
    public function rebuild()
    {
        ...
        $acl_cache = __DIR__ . $this->_filePath;

        if (!is_file($acl_cache) && is_writable(dirname($acl_cache)))
        {
            file_put_contents($acl_cache, '');
        }

        if (is_writable($acl_cache))
        {

            file_put_contents($acl_cache, serialize($acl));
